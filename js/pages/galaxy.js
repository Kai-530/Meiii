...
function initGalaxy() {
    ...
    friendsData.forEach((f) => {
        ...
        p.onclick = (e) => { 
            ...
            document.getElementById('modalBtnArea').innerHTML = `<button class="btn-main" onclick="closeAnyModal()">確認</button>`;
...
'</div>';
    ...
    document.getElementById('modalBtnArea').innerHTML = '<button class="btn-main" style="padding: 8px 25px; font-size: 0.9rem; margin-top: 5px;" onclick="closeAnyModal()">確認</button>';
    document.getElementById('modal').style.display = 'flex';
...
// ============================================================
// 統一關閉所有彈窗並重置狀態與樣式 (解決音樂未關、寬度殘留 Bug)
// ============================================================
window.closeAnyModal = function() {
    ...
    // 5. 還原 modal-card 所有的 master-card 樣式與 class
    if (modalEl) {
    ...
    if (mBody) {
        ...
        mBody.style.paddingLeft = "";
        mBody.style.paddingRight = "";
        mBody.style.boxSizing = "";
    }
};
